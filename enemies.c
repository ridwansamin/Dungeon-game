...
void BullUpdateLogic(Bull *bn, Player *P, float dt, int AttackCheck, Rectangle *AttackRect)
{
    ...
    if (P->dashing == false)
    {
        if (CheckCollisionRecs(bullrect, playerrect))
        {
            ...
            if (minOverlap == overlapleft)  { bn->x = P->x + 100; P->x -= 50; }
            if (minOverlap == overlapright) { bn->x = P->x - 200; P->x += 50; }
            if (minOverlap == overlaptop)     P->velocityY = -800.0f;
            if (minOverlap == overlapbottom)  P->velocityY =  800.0f;
